  ...
  return (
    <div className="f1-root min-h-screen flex flex-col" style={{ background: INK, color: "#fff" }}>
      <div className="fixed top-0 left-0 right-0" style={{ zIndex: 40, height: signals.length > 0 ? bannerHeight : 0 }}>
        {signals.map((s) => {
          const palette = COLORS[s.color];
          const { main, sub } = splitLabel(s.name);
          const textOutline = {
          ...
          return (
            <div
              ...
              style={{ background: palette.bg, borderBottom: `1px solid ${palette.text === "#0A0A0A" ? "#0A0A0A" : "#000"}` }}
            >
              <p
                className="font-semibold uppercase text-center"
                style={{ color: palette.text, fontSize: 24, lineHeight: 1.2, letterSpacing: "0.02em", ...textOutline }}
              >
                {s.driverName ? `${s.driverName} — ${main}` : main}
              </p>
              {sub && (
                <p
                  className="font-medium uppercase text-center"
                  style={{ color: palette.text, fontSize: 17, lineHeight: 1.2, opacity: 0.9, ...textOutline }}
                >
                  {sub}
                </p>
              )}
